...
class DQNAlgorithm(BaseAlgorithm):
    def __init__(self, n_agents, obs_shape, n_actions, config, device="cpu"):
        ...
        self.networks = [DQNNetwork(obs_shape, n_actions).to(self.device) for _ in range(n_agents)]
        self.target_networks = [DQNNetwork(obs_shape, n_actions).to(self.device) for _ in range(n_agents)]
        self.optimizers = [optim.Adam(n.parameters(), lr=self.learning_rate) for n in self.networks]
        self.target_update_freq = config.get("target_update_freq", 200)
        for n, tn in zip(self.networks, self.target_networks):
    ...
    def update(self, batch=None):
        ...
        for i in range(self.n_agents):
            ...
            nobs = torch.FloatTensor(b["next_obs"]).to(self.device)
            dones = torch.FloatTensor(b["dones"]).to(self.device)

            q = self.networks[i](obs).gather(1, act.unsqueeze(1)).squeeze(1)
            with torch.no_grad():
                max_nq = self.target_networks[i](nobs).max(dim=1)[0]
                target = rew + self.gamma * max_nq * (1 - dones)
            loss = nn.SmoothL1Loss()(q, target)
            self.optimizers[i].zero_grad()
            loss.backward()
            nn.utils.clip_grad_norm_(self.networks[i].parameters(), 10)
    ...
    def get_q_values(self, agent_id, observation):
        with torch.no_grad():
            ...
            return self.networks[agent_id](obs_t).cpu().numpy()[0]

    def state_dict(self):
        return {
            "networks": [n.state_dict() for n in self.networks],
            "target_networks": [t.state_dict() for t in self.target_networks],
            "total_steps": self.total_steps,
        }

    def load_state_dict(self, state):
        for n, s in zip(self.networks, state["networks"]):
            n.load_state_dict(s)
